		...
		if IsPedInAnyVehicle(player, false) and vehicleIsOn then
			...
			if has_value(vehiclesCars, vehicleClass) == true and vehicleClass ~= 8 then
                ...
                if not seatbeltIsOn then
                    ...
                    if (vehIsMovingFwd and (prevSpeed > (seatbeltEjectSpeed/2.237)) and (vehAcc > (seatbeltEjectAccel*3.21))) then  -- was (seatbeltEjectAccel*9.81) || this is very high.  I ran into some cars an only got about 700ish, running full speed into a head on car.  This should be about half what it is.
                        SetEntityCoords(player, position.x, position.y, position.z - 0.47, true, true, true)
                        SetEntityVelocity(player, prevVelocity.x*(vehAcc/320), prevVelocity.y*(vehAcc/320), prevVelocity.z*2)
